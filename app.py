...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tab-btn { background: #fde8f3; border: 2px solid #f9b8d8; border-radius: 24px; padding: 10px 32px; font-size: 1rem; font-weight: bold; color: #b07aad; cursor: pointer; }
  .tab-btn.active { background: #f9b8d8; color: #7a2e5e; border-color: #f490c0; }
  .tab-content { display: none; }
  .tab-content.active { display: block; }

  .drop-zone { border: 3px dashed #f9b8d8; border-radius: 20px; background: #fde8f3; padding: 40px 24px; text-align: center; max-width: 600px; margin: 0 auto32px; }
  ...
  /* チェックボックス */
  .page-checkbox-wrap { display: flex; align-items: center; justify-content: flex-start; margin-bottom: 6px; gap: 6px; }
...
<div class="container">
  ...
  <div id="tab-sort" class="tab-content active">
    ...
    <div id="sort-pages-section" style="display:none;">
      ...
      <p class="hint-text">💡 チェックボックスで選択 → 上のバーの回転ボタンでまとめて回転。☰をドラッグで並び替え。</p>
...
function loadSortPDF(file){
  ...
  reader.onload = function(e){
    ...
    pdfjsLib.getDocument({data: copy}).promise.then(function(doc){
      ...
      updateSortSelCount();
    }).catch(function(err){ showError('sort-error-msg',err); document.getElementById('loading').style.display='none'; });
  };
...
function createSortCard(orderIdx, originalIdx){
  ...
  (function(c, idx, cbEl, lbl){
    cb.addEventListener('change', function(){ updateSortSelCount(); });

    btnL.addEventListener('click', function(e){
      e.stopPropagation();
      sortRotateSingle(c, idx, -90);
    });
    ...
    //ハンドルだけdraggable ON/OFF
    handle.addEventListener('mousedown', function(){ c.draggable=true; });
    document.addEventListener('mouseup', function(){ c.draggable=false; }, {once:false});
  })(card, originalIdx, cb, cbLabel);
...
// カード個別回転（回転ボタン直押し）
function sortRotateSingle(card, idx, deg){
  sortRotations[idx] = ((sortRotations[idx]||0) + deg + 360) % 360;
  renderSortCanvas(card, idx);
}

// 選択中ページをまとめて回転（バーのボタン）
function sortRotateSelected(deg){
  var cards = Array.from(document.querySelectorAll('#sort-pages-grid .page-card'));
  var any = false;
...
function generateSortPDF(){
  ...
  }).catch(function(err){ showError('sort-error-msg',err); }).finally(function(){ btn.textContent='📥 PDFを出力する'; btn.disabled=false; });
}
...
function loadMergeFiles(files){
  ...
  chain.then(function(){ document.getElementById('merge-pages-section').style.display='block'; }).catch(function(err){ showError('merge-error-msg',err); })
...
function buildMergeSection(fd){
  ...
  hint.textContent='💡 チェックボックスで選択（複数OK）→ 上のバーの回転ボタンでまとめて回転。☰をドラッグで並び替え。';

  var grid=document.createElement('div'); grid.className='pages-grid'; grid.style.marginTop='4px';

  (function(fileData, g, cs){
    btnAll.onclick=function(){ g.querySelectorAll('.page-checkbox').forEach(function(cb){ cb.checked=true; }); updateMergeSelCount(g,cs); };
    btnNone.onclick=function(){ g.querySelectorAll('.page-checkbox').forEach(function(cb){ cb.checked=false; }); updateMergeSelCount(g,cs); };
